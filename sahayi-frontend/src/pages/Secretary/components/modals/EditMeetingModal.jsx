...
function EditMeetingModal({
  ...
  const [formData, setFormData] = useState({
    id: meeting?.id,
    title: meeting?.title || '',
    date: meeting?.date ? meeting.date.split('T')[0] : new Date().toISOString().split('T')[0],
    time: meeting?.time || '10:00 AM',
    location: meeting?.location || meeting?.venue || '',
    tag: meeting?.tag || 'NEXT WEEK'
  });
  ...
  const { hour, minute, period } = parseTimeParts(formData.time);

  const handleTimeChange = (h, m, p) => {
    setFormData({ ...formData, time: `${h}:${m} ${p}` });
  };
    ...
    <div className="sec-modal-overlay" onClick={onClose}>
      <div className="sec-modal" onClick={e => e.stopPropagation()}>
        <div className="sec-modal__header">
          <h3>Edit Scheduled Meeting</h3>
          <button className="sec-modal__close" onClick={onClose}>
            <X size={20} />
          </button>
        </div>
        <form onSubmit={handleSubmitForm} className="sec-modal__form">
